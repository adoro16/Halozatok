...
bool CMyApp::Init()
{
	...
	exit2 = glm::vec3(cos(exitAngle)*r - exit1.x, sin(exitAngle)*r - exit1.y, 0);
	Vertex ex1[] =
	{
		{ vert[randExit].p + glm::vec3(0.18,0,0.1),glm::vec3(0,0,0) },
		{ vert[randExit].p + glm::vec3(0,0.18,0.1),glm::vec3(0,0,0)},
		{ vert[randExit].p + glm::vec3(0,0,0.1), glm::vec3(0,0,0) }
...
bool CMyApp::isExit(glm::vec3 pos)
{
	if (glm::distance(pos,exit1) < 0.02 || glm::distance(pos,exit2) < 0.02)
	{
		return true;
	}
	else
...
void CMyApp::Render(glm::vec3 red, glm::vec3 blue)
{
	...
	m_matWorld = glm::translate<float>(red.x, red.y, red.z) * glm::scale<float>(1, 1, 1);
	...
	m_matWorld = glm::translate<float>(blue.x,blue.y,blue.z) * glm::scale<float>(1, 1, 1);
	glUniformMatrix4fv(m_loc_world, 1, GL_FALSE, &(m_matWorld[0][0]));
